...
# Повтор команды с растущей паузой: retry <попыток> <пауза, с> <команда...>
# Нужен там, где шаг зависит от чужой сети. Реестр образов отвечает 429, когда
# исчерпан лимит скачиваний, и та же команда через минуту проходит — ронять
# из-за этого всё развёртывание незачем.
retry() {
  ...
  while true; do
    ...
    warn "попытка $attempt из $attempts не удалась — повтор через ${delay} с"
    sleep "$delay"
    attempt=$(( attempt + 1 ))
    delay=$(( delay * 3 ))
  done
