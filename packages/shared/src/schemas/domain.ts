 ...
 *
 * Тот же список повторён в миграции `0019_domains.sql`, и синхронизировать
 * их **не нужно**: та копия разбирала имена, накопившиеся до появления
 * реестра, отработала один раз и больше не выполняется — мигратор
 * пропускает применённые файлы по отметке времени, а на чистой установке
 * ей нечего разбирать. Новые зоны дописываются только сюда.
 */
