...
describe('домены окружения', () => {
  ...
  it('отвергает IP-адрес: доменом он не является', () => {
    // Иначе корнем такого «домена» встанет последняя пара чисел («113.10»),
    // и в реестре корней заведётся запись, которую некому продлевать.
    expect(() => environmentUpdateSchema.parse({ domains: ['203.0.113.10'] })).toThrow();
    expect(() => environmentUpdateSchema.parse({ domains: ['http://10.0.0.5:3000/'] })).toThrow();
  });

  it('отвергает имя, оканчивающееся числом: такой зоны не бывает', () => {
    expect(() => environmentUpdateSchema.parse({ domains: ['example.123'] })).toThrow();
  });
